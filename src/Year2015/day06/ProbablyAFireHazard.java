...
public class ProbablyAFireHazard extends tools.RunPuzzle {
	...
	public static void main(String[] args) {
		RunPuzzle puzzle = new ProbablyAFireHazard(6, "Probably a Fire Hazard", puzzleInput);
		puzzle.run();
	}

	private int getLightCount(String[] input) {
	...
	private int getLightSum(String[] input) {
		LightGrid lights = new LightGrid();
		for (String s : input) {
			Instruction inst = new Instruction(s);
			for (int x = inst.start.x; x <= inst.end.x; x++) {
	...
	public ArrayList<TestCase> createTestCases() {
		...
		tests.add(new TestCase<String[], Integer>(1, test1, 1000000));
		tests.add(new TestCase<String[], Integer>(1, test2, 1000));
		tests.add(new TestCase<String[], Integer>(1, test3, 0));
		tests.add(new TestCase<String[], Integer>(2, test4, 1));
		tests.add(new TestCase<String[], Integer>(2, test5, 2000000));
		return tests;
	}
	...
	public Object doProcessing(int section, Object input) {
		...
		if (section == 1) {
			return getLightCount(i);
		}
		else {
			return getLightSum(i);
		}
	};
	
	static String[] test1 = {"turn on 0,0 through 999,999"};
	static String[] test2 = {"toggle 0,0 through 999,0"};
	static String[] test3 = {"turn off 499,499 through 500,500"};
	static String[] test4 = {"turn on 0,0 through 0,0"};
	static String[] test5 = {"toggle 0,0 through 999,999"};
