...
public class ExplosivesInCyberspace extends RunPuzzle {

	public ExplosivesInCyberspace(int dayNumber, String dayTitle, Object puzzleInput) {
	...
	public ArrayList<TestCase> createTestCases() {
		...
		tests.add(new TestCase<String, Long>(1, test1Input, 6l));
		tests.add(new TestCase<String, Long>(1, test2Input, 7l));
		tests.add(new TestCase<String, Long>(1, test3Input, 9l));
		tests.add(new TestCase<String, Long>(1, test4Input, 11l));
		tests.add(new TestCase<String, Long>(1, test5Input, 6l));
		tests.add(new TestCase<String, Long>(1, test6Input, 18l));
		tests.add(new TestCase<String, Long>(2, test3Input, 9l));
		tests.add(new TestCase<String, Long>(2, test6Input, 20l));
		tests.add(new TestCase<String, Long>(2, test7Input, 241920l));
		tests.add(new TestCase<String, Long>(2, test8Input, 445l));
		return tests;
	}
	...
	public Object doProcessing(int section, Object input) {
		StringBuffer data = new StringBuffer((String)input);
		long length = 0;
		while (data.length() > 0) {
			char c = data.charAt(0);
			if (c == '(') {
				int[] repeatDetails = parseRepeat(data);
				data.delete(0, repeatDetails[2]);
				
				StringBuffer toRepeat = new StringBuffer(data.substring(0, repeatDetails[0]));
				data.delete(0, repeatDetails[0]);
				
				if (section == 1 || toRepeat.indexOf("(") == -1) {
					length += repeatDetails[0] * repeatDetails[1];
				}
				else {
					length += repeatDetails[1] * (Long)doProcessing(section, toRepeat.toString());
				}
			}
			else {
				length++;
	 ...
	 *   - Number of repeats
	 *   - Number of characters in the repeat declaration
	 * @param s = StringBuffer with first character = '('
	 * @return
	 */
	private int[] parseRepeat(StringBuffer s) {
		int[] parsed = new int[3];
		
		int startIndex = 0;
		int endIndex = s.indexOf("x");
		parsed[0] = Integer.parseInt(s.substring(startIndex + 1, endIndex));
		
		startIndex = endIndex;
		endIndex = s.indexOf(")");
		parsed[1] = Integer.parseInt(s.substring(startIndex + 1, endIndex));
